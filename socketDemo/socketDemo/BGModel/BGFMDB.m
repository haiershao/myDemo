...
/**
 默认数据库名称
 */
#define SQLITE_NAME @"JYPTZData.db"
...
#define CachePath(name) [[NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:name]
...
-(void)ifIvarChangeForClass:(Class)cla ignoredKeys:(NSArray*)ignoredkeys{
    @autoreleasepool {
        ...
        [self executeDB:^(FMDatabase * _Nonnull db){
            ...
            // 2.遍历结果集
            if(rs.next){
                NSArray* columNames = [[rs resultDictionary] allKeys];
